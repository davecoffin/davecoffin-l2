<div>
    ...
    <p>I hate worrying about performance. Its the least fun part of development. Its hard enough writing code that works, are we really expected to make it work well on all devices? </p>
</div>
...
<div>
    ...
    <p>There's plenty of documentation on workers, but I found none of it went beyond the most basic example, so I'm writing this to provide a little more context. Lets get into the code.</p>
</div>
...
function processData(shifts) {
    ...
    <p>So that receives data from your main thread in the message. In the above code, workerMsg.data contains the data you send to the worker from the main thread. Then it sends a message back to the main thread with postMessage. So lets look at how to call this from the main thread:</p>
...
function processAllShifts(shifts) {
    ...
    <p>I'm using the worker to do the heavy lifting, and it returns an array with all the data that I need, then I just set the Observable Array thats the soure for my ListView to the response from the worker.</p>
...
<div>
    <p>This approach is crucial for apps that do some computationally complex operations. Its not always necessary, but dont be afraid to employ workers. In my case, when developing the app I didn't really experience any lag because I wasn't yet working with a large data set. But since my app has a bunch of users now generating a bunch of data, its more important to think about what operations may slow down the UI as the app scales.</p>
</div>
